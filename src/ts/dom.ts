...
export function initListeners(kandi: Kandi, curs: Cursor, setTool: (newTool: Tool)=>void){
	...
	addColorBtn.onclick = () => {
		...
		palette.appendChild(createPaletteItem(
			kandi.palette[kandi.palette.length-1],
			kandi.palette.length-1,
			kandi
		));
		editPalette.appendChild(createEditPaletteItem(
			kandi.palette[kandi.palette.length-1],
			kandi.palette.length-1,
			kandi
		));
 ...
 */
function childrenOfChildren(list: HTMLElement): HTMLCollection[] {
	const out: HTMLCollection[] = [];
...
function showSelection(selectedElem: HTMLElement){
	let parentElem = selectedElem.parentElement as HTMLElement;
	...
	if(parentElem.tagName === "LI"){
		...
		for(const children of childrenOfChildren(parentElem)){
			const btn = children[0] as Element;
...
function createEditPaletteItem(color: string, idx: number, kandi: Kandi): HTMLLIElement{
	...
	input.value = color;
	input.onchange = () => {
		const paletteBtn = childrenOfChildren(palette)[idx][0] as HTMLElement;
...
export function populatePalette(kandi: Kandi){
	for (let i = 0; i < kandi.palette.length; i++) {
		...
		const btn = li.firstElementChild as Element;
